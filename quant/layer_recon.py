...
def layer_reconstruction(model: QuantModel, layer: QuantModule, cali_data: torch.Tensor,
    ...
    if act_quant and order == 'together':  # 如果要量化activation且量化方式为together
        set_act_quantize_params(layer, cali_data=cached_inps[0][:min(256, cached_inps[0].size(0))], awq=True, order=order)  # 设置量化参数

    '''set state'''
    cur_weight, cur_act = weight_get_quant_state(order, act_quant)  # 当前量化方式
    layer.set_quant_state(cur_weight, cur_act)  # 设置当前层量化方式为cur_weight, cur_act

    '''set quantizer'''
    round_mode = 'learned_hard_sigmoid'  # 设置round的模式
    ...
    w_para += [layer.weight_quantizer.alpha]  # w_para为weight_quantizer的alpha
    ...
    if len(w_para) != 0:
        w_opt = torch.optim.Adam(w_para)  # w_para优化器为Adam
    if len(a_para) != 0:
        a_opt = torch.optim.Adam(a_para, lr=lr)  # a_para优化器为Adam
        a_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(a_opt, T_max=iters, eta_min=0.)  # 余弦退火
    loss_mode = 'relaxation'  #
    rec_loss = opt_mode  # 重建损失函数为opt_mode, 默认为mse
    loss_func = LossFunction(layer, round_loss=loss_mode, weight=weight,
                             max_count=iters, rec_loss=rec_loss, b_range=b_range,
                             decay_start=0, warmup=warmup, p=p)  # 损失函数
    device = 'cuda'  # 设备为gpu
    ...
    for i in range(iters):
        ...
        if input_prob < 1.0:  # probability of QDrop
            cur_inp = torch.where(torch.rand_like(cur_inp) < input_prob, cur_inp, cur_sym)  # 执行qdrop
        cur_out = cached_outs[idx].to(device)

        w_opt.zero_grad()  # 清空梯度, 方便反向传播计算
        if a_opt:
            a_opt.zero_grad()  # 清空梯度, 方便反向传播计算
        out_quant = layer(cur_inp)  # layer的量化输出

        err = loss_func(out_quant, cur_out)  # layer recon损失函数, 为该层的原输出与量化输出之间的差别

        err.backward(retain_graph=True)  # 反向传播
        w_opt.step()  # 优化器优化
        if a_opt:
            a_opt.step()  # 优化器优化
        if scheduler:
            scheduler.step()  # 优化器优化
        if a_scheduler:
            a_scheduler.step()  # 优化器优化
    torch.cuda.empty_cache()  # 清空gpu缓存

    layer.weight_quantizer.soft_targets = False  # 取消软性目标
    layer.act_quantizer.is_training = False  # 取消训练状态
    '''Case 3'''
    if act_quant and order == 'after' and waq:
        set_act_quantize_params(layer, cached_inps[0], awq=True, order=order)  # 设置量化参数
...
class LossFunction:
    def __init__(self,
        ...
        self.layer = layer
        self.round_loss = round_loss  # round损失
        self.weight = weight
        self.rec_loss = rec_loss  # 重建损失
    ...
    def __call__(self, pred, tgt, grad=None):
        ...
        if self.count < self.loss_start or self.round_loss == 'none':
            ...
            round_loss += self.weight * (1 - ((round_vals - .5).abs() * 2).pow(b)).sum()  # lambda * f_reg(V)
        else:
