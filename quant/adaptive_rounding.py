...
class AdaRoundQuantizer(nn.Module):
    ...
    def __init__(self, uaq: UniformAffineQuantizer, weight_tensor: torch.Tensor, round_mode='learned_round_sigmoid'):
        ...
        self.sym = uaq.sym
        self.delta = uaq.delta  # scale
        self.zero_point = uaq.zero_point
        self.n_levels = uaq.n_levels

        self.round_mode = round_mode  # round的方式
        self.alpha = None
    ...
    def forward(self, x):
        if self.round_mode == 'nearest':  # round to nearest
            x_int = torch.round(x / self.delta)
        elif self.round_mode == 'nearest_ste':  # ste_rounding, ste具体行为可以自己编写
            x_int = round_ste(x / self.delta)
        elif self.round_mode == 'stochastic':  # stochastic rounding
            x_floor = torch.floor(x / self.delta)
            rest = (x / self.delta) - x_floor  # rest of rounding
            x_int = x_floor + torch.bernoulli(rest)  # 把tensor每一位的值作为伯努利分布的p, 从该伯努利分布中采值作为此位的最终值
    ...
    def get_soft_targets(self):  # h(V), V是可以优化的连续变量
        # torch.sigmoid(self.alpha) * (self.zeta - self.gamma) + self.gamma = rest
